...
beta_single_plot <- ggplot(df_all2, aes(Extraction, beta_all.distances, shape = Extraction))+
  geom_boxplot(outlier.size = 0)+
  geom_beeswarm(size = 3, alpha = 0.7, aes(color = Treatment_base,shape = Extraction))+
 # geom_point(size = 3, alpha = .6, aes(shape = Extraction),position = position_jitterdodge(jitter.width = .0))+ 
  scale_color_manual(values = c("#72122A", "black","#D1686D","#EA9FB0"))+
  theme_test()+
  scale_shape(guide="none") +
...
alpha_single_plot <- ggplot(alpha_experiment, aes(Extraction, Shannon, shape = Extraction))+
  geom_boxplot(outlier.size = 0)+
    geom_beeswarm(size = 3, alpha = 0.7, aes(color = Treatment_base,shape = Extraction))+
